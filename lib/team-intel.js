...
function formatFifaRanking(row) {
  ...
  const points = Number.isFinite(row.points) ? row.points.toFixed(2) : "-";
  const movement = row.movement > 0 ? `+${row.movement}` : String(row.movement || 0);
  return `FIFA第${row.rank}，${points}分，较上期${movement}`;
}
...
function pairStatistics(homeName, awayName, homeRow, awayRow, homeFifa, awayFifa, homeRecent, awayRecent, headToHead) {
  const stats = {
    fifaRanking: {
...
async function buildTeamIntelligence(matches, { fetchImpl = fetch } = {}) {
  ...
  for (const match of matches) {
    ...
    const homeFifa = fifaRankings.get(COUNTRY_CODES[homeEn]);
    const awayFifa = fifaRankings.get(COUNTRY_CODES[awayEn]);
